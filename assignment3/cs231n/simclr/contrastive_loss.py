...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs /= torch.linalg.vector_norm(out_left, dim=1, keepdim=True) * torch.linalg.vector_norm(out_right, dim=1, keepdim=True)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    denom = torch.sum(exponential, dim=1, keepdim=True)  # shape=(2*N, 1)
    ...
    num = torch.exp(sim_positive_pairs(out_left, out_right) / tau)  # shape=(N, 1)
    ...
    numerator = num.tile(2, 1)  # shape=(2N, 1)
